// app/gestion/presupuestos/page.tsx
'use client';
...
export default function PresupuestosPage() {
  ...
  useEffect(() => {
    ...
    const fetchPresupuestos = async () => {
      try {
        const res = await fetch('/api/gestion/presupuestos', { cache: 'no-store' });
        if (!res.ok) throw new Error('Error al cargar');
        const data = await res.json();
        console.log('Presupuestos cargados:', data);
        setPresupuestos(data);
      } catch (err: any) {
        Swal.fire('Error', err.message || 'No se pudieron cargar los presupuestos', 'error');
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
        {loading ? (
          ...
          <div className="p-6 text-center text-gray-400">
            ...
            <Link href="/gestion/presupuestos/nuevo" className="text-amber-400 hover:underline">
              Crear uno nuevo
            </Link>
          </div>
        ) : (
            ...
            {presupuestos.map((p) => (
              <div key={p._id} className="p-4 hover:bg-gray-750 transition">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                  <div>
                    <div className="font-medium text-white">{p.cliente.razonSocial}</div>
                    <div className="text-sm text-gray-400">
                      ${p.total.toFixed(2)} • {new Date(p.createdAt).toLocaleDateString()}
                      {p.pedidoAsociado && (
                        <span>
                          {' • '}
                          <span className="font-bold text-amber-400">
                            Pedido #{p.pedidoAsociado.slice(-6).toUpperCase()}
                          </span>
                        </span>
                      )}
