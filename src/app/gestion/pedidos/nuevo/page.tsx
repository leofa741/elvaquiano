...
export default function NuevoPedidoPage() {
  ...
  const clienteIdFromUrl = searchParams.get('clienteId'); // 👈 leer clienteId
  ...
  // ✅ Cargar datos (solo si autorizado)
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const [resClientes, resProductos] = await Promise.all([
          ...
          fetch('/api/gestion/productos', { credentials: 'include' })
        ]);
        ...
        setProductos(
          ...
          ) || dataProductos.filter?.((p: any) => p.stock?.some((s: any) => s.cantidad > 0)) || []
        );
      } catch (err) {
  ...
  }, [clienteIdFromUrl, clientes]); // 👈 depende de clientes y clienteIdFromUrl

  // ✅ Actualizar fecha estimada (igual que antes)
  useEffect(() => {
    if (!isAuthorized) return;
    if (!clienteId) {
      setDeposito('');
  ...
  }, [isAuthorized, clienteId, clientes]);

    // ✅ Si no está autorizado, renderiza null (después de todos los hooks)
    if (!isAuthorized) {
        return null;
    }
