...
export default function AdminPage() {
  ...
  // 👇 Separación de usuarios por rol
  ...
  // 👇 Componente reutilizable para cada sección (tabla + cards)
  const renderSection = (title: string, sectionUsers: User[], emptyMsg: string) => (
    <div className="mb-10">
      <h2 className="text-xl font-bold mb-4 text-white text-center md:text-left">{title}</h2>

      {sectionUsers.length === 0 ? (
        <div className="flex items-center justify-center min-h-[200px] rounded-lg border border-gray-700 bg-gray-800">
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-6">
      ...
      {isLoading ? (
        ...
        <>
          {renderSection('Administradores', adminUsers, 'No hay administradores registrados.')}
          {renderSection('Usuarios', regularUsers, 'No hay usuarios registrados.')}
        </>
      )}
    </div>
  );
